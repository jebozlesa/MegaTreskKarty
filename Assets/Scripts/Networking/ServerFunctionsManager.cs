...
{
    private const string MatchStateFunctionMissingMessage =
        "No function named getMatchState was found to execute";
    private const string AttackCountsNotInitializedMessage =
        "Attack counts not initialized - select card first!";
    ...
    public async System.Threading.Tasks.Task<ExecuteFunctionResult> LoadPlayerDecksIntoRoomAsync(
        string playerId,
        string roomCode
    )
    {
        var tcs = new System.Threading.Tasks.TaskCompletionSource<ExecuteFunctionResult>();
        var parameters = new { playerId = playerId, roomCode = roomCode };
        CallFunction(
            "loadPlayerDecksIntoRoom",
            parameters,
            result =>
            {
    ...
    )
    {
        ...
        LogVerboseWarning(
            $"CallFunction: {functionName}, parameters: {Newtonsoft.Json.JsonConvert.SerializeObject(parameters)}"
        );
        if (ShouldAlwaysLogFunction(functionName))
        {
            Debug.Log(
                $"[ServerFunctionsManager] ExecuteFunction request: {functionName}, parameters={Newtonsoft.Json.JsonConvert.SerializeObject(parameters)}"
            );
        }
        ...
        PlayFabCloudScriptAPI.ExecuteFunction(
            ...
            {
                LogVerboseWarning(
                    $"ExecuteFunction result: {Newtonsoft.Json.JsonConvert.SerializeObject(result.FunctionResult)}"
                );
                if (ShouldAlwaysLogFunction(functionName))
                {
                    Debug.Log(
                        $"[ServerFunctionsManager] ExecuteFunction success: {functionName}, result={Newtonsoft.Json.JsonConvert.SerializeObject(result.FunctionResult)}"
                    );
                }
            ...
            error =>
            {
                ...
                {
                    Debug.LogError(
                        $"[ServerFunctionsManager] ExecuteFunction error: {functionName}, error={errorReport}"
                    );
                }
                ...
                else
                {
                    HideNetworkError();
                    Debug.LogWarning(
                        $"[ServerFunctionsManager] Suppressed network indicator for expected {functionName} error"
                    );
                }
    ...
    )
    {
        callback ??= NoOpCallback;
        LogVerboseWarning(
            $"JoinOrCreateRoom called with playerId: {playerId}, username: {username}"
        );
        var parameters = new { playerId = playerId, username = username };
    ...
    )
    {
        ...
        Debug.Log(
            $"[ServerFunctionsManager] GetMatchState requested: roomCode={roomCode}, playerId={playerId}"
        );
        var parameters = new { roomCode, playerId };
    ...
    )
    {
        ...
        Debug.Log(
            $"[ServerFunctionsManager] CancelMatchmaking requested: playerId={playerId}, roomCode={roomCode}"
        );
        var parameters = new { playerId, roomCode };
    ...
    )
    {
        callback ??= NoOpCallback;
        LogVerboseWarning(
            $"UpdatePlayerInfo called with playerId: {playerId}, username: {username}"
        );
        var parameters = new { playerId = playerId, username = username };
    ...
    public void GetRoomDecks(string roomCode, Action<ExecuteFunctionResult> callback)
    {
        ...
        var parameters = new { roomCode = roomCode };
    ...
    )
    {
        ...
        LogVerboseWarning(
            $"SetSelectedCard called with roomCode: {roomCode}, playerId: {playerId}, cardId: {cardData.cardId}"
        );
        var parameters = new
        {
            roomCode,
    ...
    )
    {
        ...
        LogVerboseWarning(
            $"[ServerFunctionsManager] GetAttackCounts: roomCode={roomCode}, playerId={playerId}, cardId={cardId}"
        );
        var parameters = new
        {
            roomCode = roomCode,
    ...
    )
    {
        ...
        var parameters = new { roomCode = roomCode, playerId = playerId };
    ...
    /// </summary>
    public void MarkReadyForNextTurn(
        string roomCode,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    ...
    /// </summary>
    public void CreateRoyalRumbleSession(
        string playerId,
        string username,
        Action<ExecuteFunctionResult> callback
    )
    ...
    public void GetRoyalRumbleSession(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"GetRoyalRumbleSession called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };

        CallFunctionWithRetry("getRoyalRumbleSession", parameters, callback);
    }

    public void LoadRoyalRumblePlayerDeck(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"LoadRoyalRumblePlayerDeck called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };
    ...
    public void LoadRoyalRumbleEnemyDeck(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"LoadRoyalRumbleEnemyDeck called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };
    ...
    public void SelectRoyalRumbleCard(
        string sessionId,
        string playerId,
        string cardId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"SelectRoyalRumbleCard called for session: {sessionId}, player: {playerId}, card: {cardId}"
        );
        var parameters = new
        {
            sessionId = sessionId,
    ...
    public void SubmitRoyalRumbleAttack(
        string sessionId,
        string playerId,
        int attackSlot,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"SubmitRoyalRumbleAttack called for session: {sessionId}, player: {playerId}, slot: {attackSlot}"
        );
        var parameters = new
        {
            sessionId = sessionId,
    ...
    public void CreateCampaignSession(
        string playerId,
        string campaignId,
        int missionId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"CreateCampaignSession called for player: {playerId}, campaign: {campaignId}, mission: {missionId}"
        );
        var parameters = new
        {
            playerId = playerId,
    ...
    public void GetCampaignProgress(
        string playerId,
        string campaignId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"GetCampaignProgress called for player: {playerId}, campaign: {campaignId}"
        );
        var parameters = new { playerId = playerId, campaignId = campaignId };

        CallFunctionWithRetry("getCampaignProgress", parameters, callback);
    }

    public void LoadCampaignPlayerDeck(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"LoadCampaignPlayerDeck called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };
    ...
    public void LoadCampaignEnemyDeck(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"LoadCampaignEnemyDeck called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };
    ...
    public void SelectCampaignCard(
        string sessionId,
        string playerId,
        string cardId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"SelectCampaignCard called for session: {sessionId}, player: {playerId}, card: {cardId}"
        );
        var parameters = new
        {
            sessionId = sessionId,
    ...
    public void SubmitCampaignAttack(
        string sessionId,
        string playerId,
        int attackSlot,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"SubmitCampaignAttack called for session: {sessionId}, player: {playerId}, slot: {attackSlot}"
        );
        var parameters = new
        {
            sessionId = sessionId,
    ...
    public void AbandonRoyalRumbleSession(
        string sessionId,
        string playerId,
        Action<ExecuteFunctionResult> callback
    )
    {
        ...
        LogVerboseWarning(
            $"AbandonRoyalRumbleSession called for session: {sessionId}, player: {playerId}"
        );
        var parameters = new { sessionId = sessionId, playerId = playerId };
    ...
    public void ShowNetworkErrorIndicator(string errorMessage = "Network error")
    {
    ...
    public void HideNetworkErrorIndicator()
    {
        HideNetworkError();
    }
        ...
        CallFunction(
            ...
            result =>
            {
                ...
                else if (retriesLeft > 0)
                {
                    // Neúspech - skús znova
                    Debug.LogWarning(
                        $"[ServerFunctionsManager] Retrying {functionName} ({retriesLeft} attempts left)..."
                    );
                    StartCoroutine(
                        RetryAfterDelay(functionName, parameters, callback, retriesLeft - 1)
                    );
                }
                else
                {
                    // Vyčerpané pokusy
                    Debug.LogError(
                        $"[ServerFunctionsManager] Failed: {functionName} failed after {maxRetries} retries!"
                    );
                    callback?.Invoke(null);
                }
            }
    ...
    private void CallMatchStateFunction(
        object parameters,
        Action<ExecuteFunctionResult> callback,
        int retriesLeft
    )
    {
        Debug.Log(
            $"[ServerFunctionsManager] ExecuteFunction request: getMatchState, parameters={Newtonsoft.Json.JsonConvert.SerializeObject(parameters)}, retriesLeft={retriesLeft}"
        );
        var request = new ExecuteFunctionRequest
        {
        ...
        PlayFabCloudScriptAPI.ExecuteFunction(
            ...
            result =>
            {
                ...
                Debug.Log(
                    $"[ServerFunctionsManager] ExecuteFunction success: getMatchState, result={Newtonsoft.Json.JsonConvert.SerializeObject(result.FunctionResult)}"
                );
                HideNetworkError();
                callback?.Invoke(result);
            },
            ...
            {
                string errorReport = error?.GenerateErrorReport() ?? string.Empty;
                bool isMissingFunction =
                    errorReport.IndexOf(
                        MatchStateFunctionMissingMessage,
                ...
                {
                    Debug.LogError(
                        "[ServerFunctionsManager] getMatchState is not registered in PlayFab."
                    );
                }

                Debug.LogWarning(errorReport);
                Debug.LogError(
                    $"[ServerFunctionsManager] ExecuteFunction error: getMatchState, error={errorReport}"
                );
                ...
                {
                    Debug.LogWarning(
                        $"[ServerFunctionsManager] Retrying getMatchState ({retriesLeft} attempts left)..."
                    );
                    StartCoroutine(
                        RetryMatchStateAfterDelay(parameters, callback, retriesLeft - 1)
                    );
                }
                else
                {
                    Debug.LogError(
                        $"[ServerFunctionsManager] Failed: getMatchState failed after {maxRetries} retries!"
                    );
                    callback?.Invoke(null);
                }
            }
    ...
    private static bool ShouldShowNetworkError(string functionName, string errorReport)
    {
        ...
        if (
            string.Equals(functionName, "getAttackCounts", StringComparison.OrdinalIgnoreCase)
            && errorReport.IndexOf(
                AttackCountsNotInitializedMessage,
                StringComparison.OrdinalIgnoreCase
            ) >= 0
        )
        {
    ...
    private IEnumerator RetryMatchStateAfterDelay(
        object parameters,
        Action<ExecuteFunctionResult> callback,
        int retriesLeft
    )
